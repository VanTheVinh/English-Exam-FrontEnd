...
const ManageQuestions = () => {
    ...
    const handleEditClick = (question) => {
        setFormData({
            ...
            questionAnswers: question.questionAnswers.length === 4 ? question.questionAnswers : Array(4).fill({ answerText: '', isCorrect: false })
        });
            ...
            fetch(`http://localhost:8000/question/${editingQuestionId}`, {
                ...
                .then(() => {
                    setQuestionsData(questionsData.map((question) =>
                        question._id === editingQuestionId ? { ...question, ...formData } : question
                    ));
        ...
        Modal.confirm({
            ...
            onOk: () => {
                fetch(`http://localhost:8000/question/${questionId}`, {
                    ...
                    .then((response) => {
                        if (response.ok) {
                            setQuestionsData(questionsData.filter((question) => question._id !== questionId));
    ...
    return (
        <Box className={styles.manage_questions}>
            ...
            {/* Form Section */}
            <Box component="form" sx={{ marginBottom: '20px' }} className={styles.box_questions}>
                <div className={styles.form_container}>
                    ...
                    <div className={styles.grid_item}>
                        <div className={styles.input_container}>
                            <label className={styles.input_label} htmlFor="difficulty">Difficulty</label>
                            <select
                                id="difficulty"
                                className={styles.input_field}
                    ...
                    <div className={styles.answers_grid}>
                        {formData.questionAnswers.map((answer, index) => (
                            <div key={index} className={styles.answer_item}>
                                <div className={styles.input_container}>
                                    <label className={styles.input_label} htmlFor={`answerText-${index}`}>Answer Text ({String.fromCharCode(65 + index)}):</label>
                                    <input
                                        type="text"
                                        id={`answerText-${index}`}
                                        name="answerText"
                                ...
                                <div className={styles.input_container}>
                                    <label className={styles.input_label} htmlFor={`isCorrect-${index}`}>Is Correct:</label>
                                    <input
                                        type="checkbox"
                                        id={`isCorrect-${index}`}
                                        name="isCorrect"
                ...
                <Table>
                    <TableHead>
                        ...
                        <TableCell style={{ textAlign: 'center', fontSize: '14px', fontWeight: 'bold' }}>#</TableCell>
                            <TableCell style={{ textAlign: 'center', fontSize: '14px', fontWeight: 'bold' }}>Exam ID</TableCell>
                            <TableCell style={{ textAlign: 'center', fontSize: '14px', fontWeight: 'bold' }}>Question Text</TableCell>
                            <TableCell style={{ textAlign: 'center', fontSize: '14px', fontWeight: 'bold' }}>Difficulty</TableCell>
                    ...
                    <TableBody>
                        {paginatedData.map((question, index) => (
                            <TableRow key={question._id}>
                                ...
                                <TableCell style={{ textAlign: 'center', fontSize: '12px' }}>{question.examId}</TableCell>
                                <TableCell style={{ textAlign: 'center', fontSize: '12px' }}>{question.questionText}</TableCell>
                                <TableCell style={{ textAlign: 'center', fontSize: '12px' }}>{question.difficulty}</TableCell>
